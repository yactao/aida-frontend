const backendUrl = 'https://aida-backend-bgp0fmd2a3c7ddf.francecentral-01.azurewebsites.net';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- FONCTIONS MISES À JOUR AVEC backendUrl ---
    async function handleAuth(url, body) {
        const errorEl = body.role ? signupError : loginError;
        if (errorEl) errorEl.textContent = '';
        try {
            const response = await fetch(`${backendUrl}${url}`, { // CORRIGÉ
                method: 'POST',
    ...
    async function fetchAndDisplayClasses() {
        ...
        try {
            const response = await fetch(`${backendUrl}/api/classes/${currentUser.email}`); // CORRIGÉ
    ...
    async function showClassDetails(classId, className) {
        ...
        try {
            const response = await fetch(`${backendUrl}/api/class/details/${classId}`); // CORRIGÉ
    ...
    async function fetchAndDisplayStudentClasses() {
        ...
        try {
            const response = await fetch(`${backendUrl}/api/student/classes/${currentUser.email}`); // CORRIGÉ
            const classes = await response.json();
    ...
    async function getAidaHelp(questionText) {
        ...
        try {
            const response = await fetch(`${backendUrl}/api/aida/help`, { // CORRIGÉ
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function loadProgrammesIntoModal() {
        ...
        try {
            const response = await fetch(`${backendUrl}/api/programmes`); // CORRIGÉ
    ...
    function setupEventListeners() {
        ...
        if (signupForm) {
            signupForm.addEventListener('submit', (e) => {
                e.preventDefault();
                handleAuth('/api/auth/signup', { // CORRIGÉ
        ...
        if (loginForm) {
            loginForm.addEventListener('submit', (e) => {
                e.preventDefault();
                handleAuth('/api/auth/login', { // CORRIGÉ
        ...
        if (createClassForm) {
            createClassForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch(`${backendUrl}/api/classes/create`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ className, teacherEmail: currentUser.email }) }); // CORRIGÉ
        ...
        if (resourceForm) {
            resourceForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch(`${backendUrl}/api/generate/quiz`, {  // CORRIGÉ
                        method: 'POST', 
                        headers: { 'Content-Type': 'application/json' }, 
        ...
        if (useQuizBtn) {
            useQuizBtn.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch(`${backendUrl}/api/class/assign-quiz`, { // CORRIGÉ
        ...
        if (joinClassForm) {
            joinClassForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch(`${backendUrl}/api/class/join`, { // CORRIGÉ
                        method: 'POST', headers: { 'Content-Type': 'application/json' },
        ...
        if (submitQuizBtn) {
            submitQuizBtn.addEventListener('click', async () => {
                ...
                try {
                    await fetch(`${backendUrl}/api/quiz/submit`, { // CORRIGÉ
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                ...
                const feedbackPromises = currentQuizData.questions.map((q, index) => {
                    ...
                    if (userAnswerIndex !== -1 && userAnswerIndex !== q.correct_answer_index) {
                        return fetch(`${backendUrl}/api/aida/feedback`, { // CORRIGÉ
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
